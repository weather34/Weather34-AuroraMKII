...
$wuskydayTempHigh2=$parsed_weather34wujson->{'daypart'}[0]->{'temperature'}[2];	 
    ...
    //mph to kts
    if ($windunit=='kts' && $wuapiunit=='e' ){$wuskydayWindGust=(number_format($wuskydayWindGust,1)*0.868976558176657);}	
    //UK mph to kts
    if ($windunit=='kts' && $wuapiunit=='h' ){$wuskydayWindGust=(number_format($wuskydayWindGust,1)*0.868976558176657);}	
    //ms to kts
    if ($windunit=='kts' && $wuapiunit=='s' ){$wuskydayWindGust=(number_format($wuskydayWindGust,1)*1.94384);}	
    ...
    //kmh to kts
    if ($windunit=='kts' && $wuapiunit=='m' ){$wuskydayWindGust1=(number_format($wuskydayWindGust1,1)*0.539957);}	
    //mph to kts
    if ($windunit=='kts' && $wuapiunit=='e' ){$wuskydayWindGust1=(number_format($wuskydayWindGust1,1)*0.868976558176657);}	
    //UK mph to kts
    if ($windunit=='kts' && $wuapiunit=='h' ){$wuskydayWindGust1=(number_format($wuskydayWindGust1,1)*0.868976558176657);}	
    //ms to kts
    if ($windunit=='kts' && $wuapiunit=='s' ){$wuskydayWindGust1=(number_format($wuskydayWindGust1,1)*1.94384);}	
    ...
    //mph to kts
    if ($windunit=='kts' && $wuapiunit=='e' ){$wuskydayWindGust2=(number_format($wuskydayWindGust2,1)*0.868976558176657);}	
    //UK mph to kts
    if ($windunit=='kts' && $wuapiunit=='h' ){$wuskydayWindGust2=(number_format($wuskydayWindGust2,1)*0.868976558176657);}
    //ms to kts
    if ($windunit=='kts' && $wuapiunit=='s' ){$wuskydayWindGust2=(number_format($wuskydayWindGust2,1)*1.94384);}	
    ...
    //mph to kts
    if ($windunit=='kts' && $wuapiunit=='e' ){$wuskydayWindGust3=(number_format($wuskydayWindGust3,1)*0.868976558176657);}	
    //UK mph to kts
    if ($windunit=='kts' && $wuapiunit=='h' ){$wuskydayWindGust3=(number_format($wuskydayWindGust3,1)*0.868976558176657);}	
    //ms to kts
    if ($windunit=='kts' && $wuapiunit=='s' ){$wuskydayWindGust3=(number_format($wuskydayWindGust3,1)*1.94384);}	
    ...
    //mph to kts
    if ($windunit=='kts' && $wuapiunit=='e' ){$wuskydayWindGust4=(number_format($wuskydayWindGust4,1)*0.868976558176657);}	
    //UK mph to kts
    if ($windunit=='kts' && $wuapiunit=='h' ){$wuskydayWindGust4=(number_format($wuskydayWindGust4,1)*0.868976558176657);}	
    //ms to kts
    if ($windunit=='kts' && $wuapiunit=='s' ){$wuskydayWindGust4=(number_format($wuskydayWindGust4,1)*1.94384);}	

    // convert cardinal to word where first is not used for different diections across languages 
    if ($wuskydayWinddircardinal=='N'){$wuskydayWinddircardinal='North';}
    if ($wuskydayWinddircardinal=='E'){$wuskydayWinddircardinal='East';}    
    if ($wuskydayWinddircardinal=='S'){$wuskydayWinddircardinal='South';}
    if ($wuskydayWinddircardinal=='Z'){$wuskydayWinddircardinal='South';}
    if ($wuskydayWinddircardinal=='W'){$wuskydayWinddircardinal='West';}
    if ($wuskydayWinddircardinal=='V'){$wuskydayWinddircardinal='West';}
//icon + day wu

echo '<div class="wudesc3day">Forecast'. ' '.$wuskydayTime.' </div>
<div class="button button-dial-smallwu" ><wutemp3day>';
//temp non metric wu
if ($weather["temp_units"]=='F'){
if($wuskydayTempHigh<44.6){echo '<blue>'.number_format($wuskydayTempHigh,0).'</blue>';}
...
echo '<div class="wu3dayhum">';
if ($wuskydaynight=='D'){
 if ($wuskydayUV>=10) {echo "<purple>".$wuskydayUV."</purple> <cbi>UVI</cbi>";}
 else if ($wuskydayUV>=8) {echo "<red>".$wuskydayUV."</red> <cbi>UVI</cbi>";}
 else if ($wuskydayUV>=5) {echo "<orange>".$wuskydayUV."</orange> <cbi>UVI</cbi>";}
 else if ($wuskydayUV>=3) {echo "<yellow>".$wuskydayUV."</yellow> <cbi>UVI</cbi>";}
 else if ($wuskydayUV>=0) {echo "<green>".$wuskydayUV."</green> <cbi>UVI</cbi>";}
}

if ($wuskydaynight=='N'){echo $weather34_humidity_iconwu."".$wuskyhumidity ."%";}
echo '</div>';
echo '<wuprob3day>'.$umbrellawu." ".$wuskydayPrecipProb.'%</wuprob3day>';
echo '<wudir3day><span style="
transform:rotate('.$wuskydayWinddir.'deg);
-webkit-transform:rotate('.$wuskydayWinddir.'deg);
-moz-transform:rotate('.$wuskydayWinddir.'deg);
-ms-transform:rotate('.$wuskydayWinddir.'deg);
-o-transform:rotate('.$wuskydayWinddir.'deg);
-ms-transform-origin: 50% 40%;
transform-origin: 50% 40%;">
'.$weather34compassiconwu ."</span>&nbsp;".$wuskydayWinddircardinal.'</wudir3day>';
echo '<wuwind3day>';
//wind/gusts
if($windunit=='mph' && $wuskydayWindGust>=30){echo "<red>".$weather34_wind_iconwu."&nbsp;" .number_format($wuskydayWindGust*1.625,0)."</red> ";}
else if($windunit=='mph' && $wuskydayWindGust>=18){echo "<orange>".$weather34_wind_iconwu."&nbsp;" .number_format($wuskydayWindGust*1.625,0)."</orange> ";}
else if ($wuskydayWindGust>=30) {echo "<red>".$weather34_wind_iconwu."&nbsp;" .number_format($wuskydayWindGust*1.625,0)."</red> ";}
else if ($wuskydayWindGust>=25) {echo "<orange>".$weather34_wind_iconwu."&nbsp;" .number_format($wuskydayWindGust*1.625,0)."</orange> ";}
else if ($wuskydayWindGust>=20) {echo "<yellow>".$weather34_wind_iconwu."&nbsp;" .number_format($wuskydayWindGust,0)."</yellow> ";}
else if ($wuskydayWindGust>=0) {echo "<green>".$weather34_wind_iconwu."&nbsp;" .number_format($wuskydayWindGust,0)."</green> ";}
echo "<wuiunit>&nbsp;".$windunit.'</wuiunit></wuwind3day>'; 
?>

</div></div>
<div class="wuicon3d"><?php
if ($wuskydaynight=='D'){echo '<img src="wuicons/'.$wuskydayIcon.'.svg" width="45" height="35" alt="'.$wuskydesc.'" title="'.$wuskydesc.'"></img>';}
if ($wuskydaynight=='N'){echo '<img src="wuicons/nt_'.$wuskydayIcon.'.svg" width="45" height="35" alt="'.$wuskydesc.'" title="'.$wuskydesc.'"></img>';}
// icon description
$wuskydesc	=str_replace('Wind',"Windy Conditions", $wuskydesc);
$wuskydesc	=str_replace('/', ' ', $wuskydesc);echo '</div>
<div class="wuicondesc3d">'.$wuskydesc.'</div>';?>
</div></div>

<?php
echo '<div class=wuaheadfore>';
if ($wuskythunder>0 )  {echo "Thunderstorms" .$wuskydayTime.'&nbsp;'.$lightningalert8.'';}
