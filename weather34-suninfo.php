<?php 
  ######################################################### 
	#	Aurora MKII Sun Info data by BRIAN UNDERDOWN 2015-21       
...
$darkhours = 23 - $lighthours; $darkminutes = 60 - $lightmins;if ($darkminutes<10) $darkminutes= '0' .$darkminutes;
   ...
   <?php 
$time = time();$longitude = $lon;$latitude = $lat;$gmtOffset =$UTC;$zenith = 90+(50/60);
...
foreach ($results as $value) {if (gettype($value) !== 'integer') {$incorrectDataValues++;}
}
?>
...
<head>
  <meta charset="UTF-8">
  <title>Weather34 Aurora MKII Sun Position Information</title>
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <link href="console-dark.css?version=<?php echo filemtime('console-dark.css') ?>" rel="stylesheet prefetch">
  <style>
.grida{display:grid;grid-template-columns:repeat(3,1fr);grid-column-gap:3px;grid-row-gap:3px;color:#afb7c0;margin-top:5px}
.grida>article{padding:5px;font-size:9px;border-radius:4px;-webkit-border-radius:4px;background:0;-webkit-font-smoothing:antialiased;-moz-osx-font-smoothing:grayscale;border:1px solid hsla(217,15%,17%,.5);border-bottom:10px solid hsla(217,15%,17%,.5);height:155px;font-family:verb}
...
.weather34-container-clock{width:120px;height:120px;position:relative;top:-40px;left:-45px}
...
.clock34{box-shadow:var(--button-shadow);background-color:#1d2124;display:flex;font-family:clock;border-radius:3px;-webkit-border-radius:3px;-moz-border-radius:3px;-ms-border-radius:3px;-o-border-radius:3px;left:0;text-align:center;position:relative;color:hsla(214,29%,91%,.6);font-size:1rem;text-transform:none;padding:4px;padding-top:6px;top:-147px;font-family:clock;z-index:20;width:5.5em;background-image:linear-gradient(hsla(0,0%,33%,.1) 1px,transparent 1px),linear-gradient(to right,hsla(0,0%,33%,.1) 1px,transparent 1px);background-size:5px 5px;justify-content:center;align-items:center}
...
.weather34weekdays{position:relative;text-transform:uppercase;font-size:.8rem;display:grid;grid-template-columns:repeat(7,35px);grid-gap:.3em;list-style:none;left:6px;top:-70px;margin-bottom:-145px}
...
<article>
<?php echo $info;?> Sun Position | Moonphase<br>
<div style="position:relative;margin-left:175px;margin-top:70px"> 
<div id="sunpos"></div></div>
</article>

<article>
<?php echo $info;?> Sunrise/Set | Moonrise/Set Times<br>
<div style="position:absolute;margin-left:5px;margin-top:20px;width:max-content">
<?php echo $sunuphalf." First Light (<deepblue>".date('H:i',$civil_twilight_morning)."</deepblue>)";?><br>
<?php echo $sunuphalf1.' Sunrise';?><?php echo $nextrisetxt.' (<yellow>'.$nextrise.'</yellow>)';?><br>
<?php echo $sunuphalf.' Total Daylight (<deepblue>'.$light.'</deepblue> Hours)';?><br><br>
<?php echo $sundownhalf1.' Sunset';?> <?php echo $nextsettxt.' (</deepblue><orange>'.$nextset.'</orange>)';?><br>
...
<article>
<?php echo $info;?> Approximate Sun Azimuth/Elevation<br>
<div style="position:absolute;margin-left:5px;margin-top:20px;width:max-content">

<?php if ($elev<=0){echo $sundownhalf1;}else if ($elev>0){echo $sunuphalf1;};echo ' Sun Azimuth';?> 
<deepblue><?php echo $azimuth ?>&deg;</deepblue>
<br>
<?php 
if ($elev<=0){echo $sundownhalf1;}else if ($elev>0){echo $sunuphalf1;};echo ' Sun Elevation';?>  
<deepblue><?php echo $elev ?>&deg;</deepblue>  <br>
...
<article>  
<div style="font-size:9px;"><?php echo $info;?> Local Time</div>
...
<div style="margin-top:25px;margin-left:200px;" id="weather34clock2"></div>
