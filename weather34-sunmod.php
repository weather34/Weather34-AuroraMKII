...
if ($now > $nextfirstlight) {$nextfirstlight = date('H:i',$result2['civil_twilight_begin']); } 
...
if ($now < $firstrise) { $time = $firstrise - $now; $hrs = gmdate ('G',$time); $min = gmdate ('i',$time); $txt = 'Darkness';$txt2 = "Darkness";} 
elseif ($now < $firstset) { $time = $firstset - $now; $hrs = gmdate ('G',$time); $min = gmdate ('i',$time); $txt = 'Daylight';$txt2 = "Daylight";} 
else { $time = $secondrise - $now; $hrs = gmdate ('G',$time); $min = gmdate ('i',$time);$txt ='Sunrise';$txt2 = "Darkness";};
...
$darkhours = 23 - $lighthours; $darkminutes = 60 - $lightmins;if ($darkminutes<10) $darkminutes= '0' .$darkminutes;
 ...
 <div class="sunblock"><div class="daylighthours">Sunrise | Sunset</div>
<div class="button button-dial-small-sun">
<div class="button-dial-label-small-hours"><?php echo $txt?></div>
